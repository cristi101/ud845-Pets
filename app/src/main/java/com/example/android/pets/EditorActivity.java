...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor>, View.OnTouchListener {
    ...
    private void savePet() {
        ...
        if (mUri != null) {
            int result = resolver.update(mUri, values, null, null);
            if (result == 0) Toast.makeText(this, "Error updating pet", Toast.LENGTH_LONG).show();
            else Toast.makeText(this, "Pet updated", Toast.LENGTH_LONG).show();
        } else {
            // Insert a new row for pet in the database, returning the ID of that new row.
            Uri newRowId = resolver.insert(PetEntry.CONTENT_URI, values);
            ...
            if (newRowId == null) {
                ...
                Toast.makeText(this, "Pet saved with row id: " + newRowId.toString(), Toast.LENGTH_SHORT).show();
            }
        }
    }
    ...
    private void deletePet() {
        ...
        ContentResolver resolver = getContentResolver();
        int result = resolver.delete(mUri, null, null);
        if (result == 0) {
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        if (data.moveToFirst()) {
            String nameString = data.getString(1);
            String breedString = data.getString(2);
            String weightString = data.getString(4);
            int weight = Integer.parseInt(weightString);
            mGender = data.getInt(3);
